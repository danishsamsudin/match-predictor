...
export function resolveKnockoutMatchOutcomeQuick(
  ...
  let homeGoals = Math.max(0, Math.round(homeXg));
  let awayGoals = Math.max(0, Math.round(awayXg));
  ...
  const etHomeGoals = Math.round(homeXg * 0.3);
  const etAwayGoals = Math.round(awayXg * 0.3);
  homeGoals += etHomeGoals;
  awayGoals += etAwayGoals;
