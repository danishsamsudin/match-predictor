#!/usr/bin/env python3
"""
Train / estimate GLPM hierarchical Build-Up Ratings (Chapter 6).

Usage (from repo root):
  python3 scripts/glpm_train_build_up_ratings.py --synthetic
  python3 scripts/glpm_train_build_up_ratings.py --season-id 12345 --dry-run
"""
                ...
                rows.append(
                    {
                        ...
                        "progressive_passes": int(rng.integers(20, 60) * strength),
                        "progressive_carries": int(rng.integers(10, 40)),
                        "final_third_entries": int(rng.integers(20, 50) * strength),
                        "box_entries": int(rng.integers(15, 40)),
                        "through_balls": int(rng.integers(1, 8)),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train GLPM Build-Up Ratings")
    ...
    pipeline = BuildUpRatingPipeline()
    ...
    if args.synthetic:
        ...
        summary = result.team_summary[
            ["team_sm_id", "rating_build_up", "rating_domain_progression"]
        ].sort_values("rating_build_up", ascending=False)
    ...
    _load_dotenv()
    from models.ratings.build_up.io import get_supabase_client
        ...
        json.dumps(
            {
                ...
                "model_version": result.model_version,
                "dry_run": args.dry_run,
                "top": result.team_summary.sort_values("rating_build_up", ascending=False)
                .head(5)[["team_sm_id", "rating_build_up"]]
