/**
 * Seed Quarter-final fixtures into Supabase `matches` for hub predictions.
 *
 * Usage: npx tsx scripts/seed-world-cup-qf-fixtures.ts [--dry-run]
 */
...
import {
  buildQfHubMatchRows,
  loadQfFixtures,
  qfFixtureHasBothTeams,
  qfMatchId,
} from "../src/lib/world-cup/qf-hub-fixtures";
...
async function main() {
  ...
  const rows = buildQfHubMatchRows(teamNames);
  let upserted = 0;
  let skipped = 0;
  const warnings: string[] = [];

  for (const fx of loadQfFixtures()) {
    const row = rows.find((m) => m.id === qfMatchId(fx.match_number));
    if (!row) continue;

    if (!qfFixtureHasBothTeams(fx)) {
      skipped += 1;
      warnings.push(
  ...
  console.log(
    `${dryRun ? "[dry-run] " : ""}Upserted ${upserted} QF fixtures, skipped ${skipped} with placeholders`
  );
