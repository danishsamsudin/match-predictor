/**
 * Import Quarter-final fixtures from a saved FIFA "Game Schedule & Where to Watch" HTML page.
 *
 * Usage:
 *   npx tsx scripts/import-fifa-qf-schedule.ts [path-to-html] [--dry-run]
 *
 * Default HTML path:
...
const OUT_PATH = path.join(process.cwd(), "data/world-cup-2026/qf-fixtures.json");
const BRACKET_PATH = path.join(process.cwd(), "data/world-cup-2026/knockout-bracket.json");

function loadQfBracketFallbacks(): FifaKnockoutScheduleFallback[] {
...
function main() {
  ...
  const fallbacks = loadQfBracketFallbacks();
  const fixtures = parseFifaWtwQfScheduleHtml(html, fallbacks);

  if (fixtures.length !== 4) {
    console.error(`Expected 4 Quarter-final fixtures, parsed ${fixtures.length}`);
    process.exit(1);
  }

  const payload = {
    version: "qf-live-draw-v1",
  ...
  console.log(`Wrote ${fixtures.length} QF fixtures to ${OUT_PATH}`);
