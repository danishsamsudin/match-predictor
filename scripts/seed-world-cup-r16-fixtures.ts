/**
 * Seed Round of 16 fixtures into Supabase `matches` for hub predictions.
 *
 * Usage: npx tsx scripts/seed-world-cup-r16-fixtures.ts [--dry-run]
 */
import fs from "node:fs";
import path from "node:path";
import { normalizeNationalTeamName } from "../src/lib/data/world-cup-2026-teams";
import {
  buildR16HubMatchRows,
  loadR16Fixtures,
  r16FixtureHasBothTeams,
  r16MatchId,
} from "../src/lib/world-cup/r16-hub-fixtures";
import { isKnockoutSlotPlaceholder } from "../src/lib/world-cup/r32-hub-fixtures";
...
async function main() {
  ...
  const rows = buildR16HubMatchRows(teamNames);
  let upserted = 0;
  let skipped = 0;
  const warnings: string[] = [];

  for (const fx of loadR16Fixtures()) {
    const row = rows.find((m) => m.id === r16MatchId(fx.match_number));
    if (!row) continue;

    if (!r16FixtureHasBothTeams(fx)) {
      skipped += 1;
      warnings.push(
    ...
    if (dryRun) {
      console.log(
        `[dry-run] ${payload.id} ${payload.date} ${fx.home_team} vs ${fx.away_team} @ ${payload.venue_city} (${payload.status})`
      );
      upserted += 1;
      continue;
    }
  ...
  console.log(
    `${dryRun ? "[dry-run] " : ""}Upserted ${upserted} R16 fixtures, skipped ${skipped} with placeholders`
  );
