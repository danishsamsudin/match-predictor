"""
End-to-end Finishing Rating pipeline (Chapter 9).
...
from features.finishing import build_finishing_features
from models.ratings.finishing.adjust import OpponentContextAdjuster
from models.ratings.finishing.components import COMPONENT_NAMES, ComponentRatingSuite
from models.ratings.finishing.domains import DOMAIN_COMPONENT_MAP, DomainRatingSuite
from models.ratings.finishing.primary import PrimaryFinishingModel
...
class FinishingRatingPipeline:
    def __init__(
        ...
        *,
        ...
        self.primary = PrimaryFinishingModel(
    ...
    def run_from_frames(
        ...
        *,
        ...
        if "goals_minus_xg" in adjusted.columns and "s_opp" in adjusted.columns:
            adjusted["goals_minus_xg_adj"] = adjusted["goals_minus_xg"].astype(float) / adjusted[
                "s_opp"
        ...
        return FinishingPipelineResult(
            match_frame=scored,
    ...
    def run_from_supabase(
        ...
        *,
        ...
        from models.ratings.finishing.io import (
    ...
    def _calibrate_all(
        ...
        cal = GlpmCalibrator(version=self.model_version)
        scores = out["finishing_latent"].astype(float).to_numpy()
        finite = scores[np.isfinite(scores)]
        cal.fit(finite)
        out["rating_finishing"] = [
            cal.transform_one(float(s)) if np.isfinite(s) else np.nan for s in scores
        ]
        calibrators["finishing"] = cal
    ...
    def _aggregate_team_ratings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, row in latest.iterrows():
            ...
            rec: dict[str, Any] = {
                ...
                "rating_finishing": float(row["rating_finishing"]),
                "conf_finishing": float(row.get("finishing_confidence", 0.5)),
                "var_finishing": float(row.get("finishing_variance", 0.0)),
            }
            for name in DOMAIN_COMPONENT_MAP:
    ...
    def _save_artifacts(self, scored: pd.DataFrame) -> None:
        ...
        joblib.dump(
            ...
            self.artifact_dir / "finishing_models.joblib",
        )
        scored[
            [
                c
