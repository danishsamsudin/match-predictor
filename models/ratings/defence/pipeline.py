"""
End-to-end Defence Rating pipeline (Chapter 4).
...
from features.defence import build_defence_features
from models.ratings.defence.adjust import OpponentContextAdjuster
from models.ratings.defence.components import COMPONENT_NAMES, ComponentRatingSuite
from models.ratings.defence.domains import DOMAIN_COMPONENT_MAP, DomainRatingSuite
from models.ratings.defence.primary import PrimaryDefenceModel
...
class DefenceRatingPipeline:
    def __init__(
        ...
        *,
        ...
        self.primary = PrimaryDefenceModel(future_window=future_window, model_version=model_version)
    ...
    def run_from_frames(
        ...
        *,
        ...
        if "xga_suppression" in adjusted.columns and "s_opp" in adjusted.columns:
            adjusted["xga_suppression_adj"] = adjusted["xga_suppression"].astype(float) / adjusted[
                "s_opp"
    ...
    def run_from_supabase(
        ...
        *,
        ...
        from models.ratings.defence.io import (
            load_l2_features,
    ...
    def _calibrate_all(
        ...
        cal = GlpmCalibrator(version=self.model_version)
        scores = out["defence_latent"].astype(float).to_numpy()
        finite = scores[np.isfinite(scores)]
        cal.fit(finite)
        out["rating_defence"] = [
            cal.transform_one(float(s)) if np.isfinite(s) else np.nan for s in scores
        ]
        calibrators["defence"] = cal
    ...
    def _aggregate_team_ratings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, row in latest.iterrows():
            ...
            rec: dict[str, Any] = {
                ...
                "rating_defence": float(row["rating_defence"]),
                "conf_defence": float(row.get("defence_confidence", 0.5)),
                "var_defence": float(row.get("defence_variance", 0.0)),
            }
            for name in DOMAIN_COMPONENT_MAP:
    ...
    def _save_artifacts(self, scored: pd.DataFrame) -> None:
        ...
        joblib.dump(
            ...
            self.artifact_dir / "defence_models.joblib",
        )
        scored[
            [
                c
