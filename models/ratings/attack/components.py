"""
Attack Component Rating estimators (Chapter 3.9–3.14).
...
from features.attack import COMPONENT_FEATURE_GROUPS

LIGHTGBM_MIN_ROWS = 500

COMPONENT_TARGET_MAP: dict[str, str] = {
    "chance_volume": "shots_p90_adj",
    "chance_quality": "xg_per_shot_adj",
    "ball_progression": "prog_pass_rate_adj",
    "territorial_control": "field_tilt_adj",
    "transition_threat": "transition_xg_per_recovery_adj",
    "set_piece_threat": "set_piece_xg_per_match_adj",
}
...
class ComponentModel:
    ...
    def fit(self, df: pd.DataFrame, y: np.ndarray) -> "ComponentModel":
        ...
        mask = np.isfinite(y)
        if mask.sum() < 5:
            # Degenerate: predict column mean of available features
    ...
    def predict_with_uncertainty(self, df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
        ...
        # Fallback: use residual scale proxy
    ...
    def prepare_targets(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for name, model in self.models.items():
            ...
            if target_raw not in out.columns:
                # Fall back to non-adjusted feature
