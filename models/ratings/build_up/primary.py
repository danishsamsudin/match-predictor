"""
Primary Build-Up Rating BU_i (Chapter 6).

BU = f(Progression, Retention, Distribution)
Supervised on future adjusted progression outcomes.
"""
...
DOMAIN_INPUT_COLS = ("domain_progression", "domain_retention", "domain_distribution")
PRIMARY_TARGET_CANDIDATES = (
    "final_third_entry_rate_adj",
    "prog_pass_rate_adj",
...
def resolve_primary_target(df: pd.DataFrame) -> str:
    for col in PRIMARY_TARGET_CANDIDATES:
        if col in df.columns and df[col].notna().any():
            return col
    raise ValueError("No primary build-up target column available")


@dataclass
class PrimaryBuildUpModel:
    estimator: Any = None
    scaler: StandardScaler = field(default_factory=StandardScaler)
    target_col: str = "final_third_entry_rate"
    future_window: int = 5
    fitted: bool = False
    model_version: str = "build_up_v1"
    ...
    def fit(self, df: pd.DataFrame) -> "PrimaryBuildUpModel":
    ...
    def predict(self, df: pd.DataFrame) -> np.ndarray:
        if not self.fitted or self.estimator is None:
            raise RuntimeError("Primary Build-Up model is not fitted")
        return np.asarray(
