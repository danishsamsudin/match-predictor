"""
Opponent & context adjustment for Build-Up features (Chapter 6.8).
...
from features.midfield import ALL_BUILD_UP_FEATURES
...
class OpponentContextAdjuster:
    ...
    feature_cols: Sequence[str] = field(default_factory=lambda: list(ALL_BUILD_UP_FEATURES))
...
def adjusted_feature_names(feature_cols: Iterable[str] | None = None) -> list[str]:
    cols = list(feature_cols) if feature_cols is not None else list(ALL_BUILD_UP_FEATURES)
