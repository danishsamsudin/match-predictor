"""
Primary Pressing Rating PR_i (Chapter 8).

PR = f(Press Intensity, Ball Recovery, Press Effectiveness)
Supervised on future adjusted pressing outcomes.
"""
...
DOMAIN_INPUT_COLS = (
    "domain_press_intensity",
    "domain_ball_recovery",
    "domain_press_effectiveness",
)
PRIMARY_TARGET_CANDIDATES = (
    "ppda_inv_adj",
    "high_turnovers_p90_adj",
    "ppda_inv",
    "high_turnovers_p90",
)
...
def resolve_primary_target(df: pd.DataFrame) -> str:
    for col in PRIMARY_TARGET_CANDIDATES:
        if col in df.columns and df[col].notna().any():
            return col
    raise ValueError("No primary pressing target column available")


@dataclass
class PrimaryPressingModel:
    estimator: Any = None
    scaler: StandardScaler = field(default_factory=StandardScaler)
    target_col: str = "ppda_inv"
    future_window: int = 5
    fitted: bool = False
    model_version: str = "pressing_v1"
    ...
    def fit(self, df: pd.DataFrame) -> "PrimaryPressingModel":
    ...
    def predict(self, df: pd.DataFrame) -> np.ndarray:
        if not self.fitted or self.estimator is None:
            raise RuntimeError("Primary Pressing model is not fitted")
        return np.asarray(
