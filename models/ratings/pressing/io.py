"""
Supabase IO for Pressing Rating training and persistence.
"""
...
DOMAINS = ("press_intensity", "ball_recovery", "press_effectiveness")
COMPONENTS = (
    "high_press",
    "mid_block_press",
    "counter_press",
...
def get_supabase_client():
    ...
    if not url or not key:
        raise RuntimeError(
            "Missing NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY for Pressing ratings IO"
        )
    ...
    try:
        ...
        if press_rows:
            ...
            latest = latest.rename(
                columns={"team_sm_id": "opponent_team_sm_id", "rating": "build_up_rating"}
            )[["opponent_team_sm_id", "build_up_rating"]]
        ...
        primary_rows.append(
            {
                **base,
                ...
                "rating": float(row["rating_pressing"]),
                "confidence": float(row.get("conf_pressing", 0.5)),
                "variance": float(row.get("var_pressing", 0.0)),
            }
        )
        history_rows.append(
            {
