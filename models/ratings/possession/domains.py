"""
Possession Domain Ratings (Chapter 7).

Ball Retention      = f(Possession Security, Ball Circulation)
Territorial Control = f(Territorial Dominance, Space Control)
Possession Control  = f(Game Control, Possession Tempo)
"""
...
DOMAIN_COMPONENT_MAP: dict[str, tuple[str, str]] = {
    "ball_retention": ("possession_security", "ball_circulation"),
    "territorial_control": ("territorial_dominance", "space_control"),
    "possession_control": ("game_control", "possession_tempo"),
}

DOMAIN_TARGET_MAP: dict[str, str] = {
    "ball_retention": "ball_security_index_adj",
    "territorial_control": "field_tilt_adj",
    "possession_control": "match_control_index_adj",
}
...
class DomainRatingSuite:
    ...
    def prepare_targets(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for name, model in self.models.items():
            ...
            out[f"domain_{name}__y"] = future_rolling_mean(out, col, window=self.future_window)
        return out

    def fit(self, df: pd.DataFrame) -> "DomainRatingSuite":
        frame = self.prepare_targets(df)
        for name, model in self.models.items():
            model.fit(frame, frame[f"domain_{name}__y"].astype(float).to_numpy())
        return self
