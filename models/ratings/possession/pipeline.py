"""
End-to-end Possession Rating pipeline (Chapter 7).
...
from features.midfield import build_possession_features
from models.ratings.possession.adjust import OpponentContextAdjuster
from models.ratings.possession.components import COMPONENT_NAMES, ComponentRatingSuite
from models.ratings.possession.domains import DOMAIN_COMPONENT_MAP, DomainRatingSuite
from models.ratings.possession.primary import PrimaryPossessionModel
from models.ratings.scale import GlpmCalibrator

ARTIFACT_DIR = Path(__file__).resolve().parent / "artifacts"
MODEL_VERSION = "possession_v1"
...
class PossessionRatingPipeline:
    def __init__(
        self,
        *,
        ...
        self.primary = PrimaryPossessionModel(future_window=future_window, model_version=model_version)
    ...
    def run_from_frames(
        ...
        *,
        ...
        feats = build_possession_features(match_frame, l2_frame=l2_frame)
        for col in (
            "ppda",
            "high_turnovers",
            "opp_ppda",
        ...
        if "possession_pct" in adjusted.columns and "s_opp" in adjusted.columns:
            adjusted["possession_pct_adj"] = adjusted["possession_pct"].astype(float) / adjusted[
                "s_opp"
        ...
        return PossessionPipelineResult(
            match_frame=scored,
    ...
    def run_from_supabase(
        ...
        *,
        ...
        from models.ratings.possession.io import (
            load_l2_features,
    ...
    def _calibrate_all(
        ...
        cal = GlpmCalibrator(version=self.model_version)
        scores = out["possession_latent"].astype(float).to_numpy()
        finite = scores[np.isfinite(scores)]
        cal.fit(finite)
        out["rating_possession"] = [
            cal.transform_one(float(s)) if np.isfinite(s) else np.nan for s in scores
        ]
        calibrators["possession"] = cal
    ...
    def _aggregate_team_ratings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, row in latest.iterrows():
            ...
            rec: dict[str, Any] = {
                ...
                "rating_possession": float(row["rating_possession"]),
                "conf_possession": float(row.get("possession_confidence", 0.5)),
                "var_possession": float(row.get("possession_variance", 0.0)),
            }
            for name in DOMAIN_COMPONENT_MAP:
    ...
    def _save_artifacts(self, scored: pd.DataFrame) -> None:
        ...
        joblib.dump(
            ...
            self.artifact_dir / "possession_models.joblib",
        )
        scored[
            [
                c
