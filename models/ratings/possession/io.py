"""
Supabase IO for Possession Rating training and persistence.
"""
...
RATING_TYPE = "possession"

DOMAINS = ("ball_retention", "territorial_control", "possession_control")
COMPONENTS = (
    "possession_security",
    "ball_circulation",
...
def get_supabase_client():
    ...
    if not url or not key:
        raise RuntimeError(
            "Missing NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY for Possession ratings IO"
        )
        ...
        primary_rows.append(
            {
                **base,
                ...
                "rating": float(row["rating_possession"]),
                "confidence": float(row.get("conf_possession", 0.5)),
                "variance": float(row.get("var_possession", 0.0)),
            }
        )
        history_rows.append(
            {
