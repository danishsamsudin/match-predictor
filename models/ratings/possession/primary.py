"""
Primary Possession Rating PO_i (Chapter 7).

PO = f(Ball Retention, Territorial Control, Possession Control)
Supervised on future adjusted possession outcomes.
"""
...
DOMAIN_INPUT_COLS = ("domain_ball_retention", "domain_territorial_control", "domain_possession_control")
PRIMARY_TARGET_CANDIDATES = (
    "possession_pct_adj",
    "ball_security_index_adj",
    "possession_pct",
    "ball_security_index",
)
...
def resolve_primary_target(df: pd.DataFrame) -> str:
    for col in PRIMARY_TARGET_CANDIDATES:
        if col in df.columns and df[col].notna().any():
            return col
    raise ValueError("No primary possession target column available")


@dataclass
class PrimaryPossessionModel:
    estimator: Any = None
    scaler: StandardScaler = field(default_factory=StandardScaler)
    target_col: str = "possession_pct"
    future_window: int = 5
    fitted: bool = False
    model_version: str = "possession_v1"
    ...
    def fit(self, df: pd.DataFrame) -> "PrimaryPossessionModel":
    ...
    def predict(self, df: pd.DataFrame) -> np.ndarray:
        if not self.fitted or self.estimator is None:
            raise RuntimeError("Primary Possession model is not fitted")
        return np.asarray(
