"""
Primary Goalkeeper Rating GK_i (Chapter 5 Part VI).

GK_i = f(Goal Prevention, Goalkeeper Involvement)
Supervised on future adjusted Goals Prevented (PSxG − GC).
"""
...
DOMAIN_INPUT_COLS = ("domain_goal_prevention", "domain_goalkeeper_involvement")
...
def resolve_primary_target(df: pd.DataFrame) -> str:
    for col in PRIMARY_TARGET_CANDIDATES:
        if col in df.columns and df[col].notna().any():
            return col
    raise ValueError("No primary goalkeeper target column available")


@dataclass
class PrimaryGoalkeeperModel:
    estimator: Any = None
    scaler: StandardScaler = field(default_factory=StandardScaler)
    target_col: str = "goals_prevented"
    future_window: int = 5
    fitted: bool = False
    model_version: str = "gk_v1"
    ...
    def prepare_target(self, df: pd.DataFrame) -> pd.Series:
        ...
        return future_rolling_mean(
            df, self.target_col, group_col=self.group_col, window=self.future_window
        )

    def fit(self, df: pd.DataFrame) -> "PrimaryGoalkeeperModel":
    ...
    def predict(self, df: pd.DataFrame) -> np.ndarray:
        if not self.fitted or self.estimator is None:
            raise RuntimeError("Primary Goalkeeper model is not fitted")
        return np.asarray(
    ...
    def score(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out["gk_latent"] = pred
        out["gk_latent_std"] = std
        n_by_player = (
            out.groupby(self.group_col)["match_sm_id"].transform("count").astype(float)
        )
        conf = 1.0 / (1.0 + std) * np.clip(n_by_player / 20.0, 0.2, 1.0)
        out["gk_confidence"] = conf
        out["gk_variance"] = std ** 2
        return out
